    ...
    def _loss_on(self, batch, flags, plan, want_logs=True):
        ...
        if plan.use_self_supervised_loss:
            ...
            if want_logs:
                ...
                if getattr(plan, 'token_ground_weight', 0.0) * getattr(self.train_cfg, 'ssl_token_weight', 0.0) > 0:
                    # token-grounded next-chunk NLL (train/serve exposure fix); the
                    # value that MUST fall if the Talker is learning to decode the
